  ...
  <header id="topbar">
    <el-row>
      <el-col :span="4" class="topbar-left">
        <i class="iconfont "></i>
        <span class="title" @click="index()">ncwu在线考试系统</span>
      </el-col>
      <el-col :span="20" class="topbar-right">
        <i class="el-icon-menu" @click="toggle()"></i>
        <div class="user">
          <span>{{user.userName}}</span>
          <img src="@/assets/img/zzx.jpeg" class="user-img" ref="img" @click="showSetting()" />
...
export default {
  ...
  methods: {
    ...
    getUserInfo() { //获取用户信息
      ...
      let usersex = this.$cookies.get("csex")
      let usertel = this.$cookies.get("ctel")
      this.user.userName = userName
      this.user.userId = userId
      this.user.usersex = usersex
      this.user.usertel = usertel
