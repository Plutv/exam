...
<template>
  <div id="answer">
    <!--顶部信息栏-->
     <div class="top">
       <ul class="item">
         <li><i class="" ref="toggle" @click="slider_flag = !slider_flag"></i></li>
     ...
     <div class="flexarea">
        ...
        <transition name="slider-fade">
          <div class="left" v-if="slider_flag">
            ...
            <div class="l-bottom">
              <!--动态绑定class的属性。根据条件表达式的结果，决定是否为链接元素添加特定的css类,{{number}}是Vue的插值语法,从Vue组件中获取的一个变量或表达式的值，将其显示在模板中-->
              <div class="item">
                <p>选择题部分</p>
                <ul>
        ...
        <div class="right">
          <div class="title">
            <p>{{title}}</p>
            <i class="  auto-right"></i>
            <span>  <i class=""></i>倒计时：<b>{{time}}</b>分钟</span>
          </div>
          <div class="content">
...
export default {
  ...
  methods: {
    ...
    getCookies() {  //获取cookie
      ...
      this.userInfo.sex = this.$cookies.get("csex")
    },
    calcuScore() { //计算答题分数
    ...
    getExamData() { //获取当前试卷所有信息
      ...
      this.$axios(`/api/exam/${examCode}`).then(res => {  //通过examCode请求试卷详细信息
        ...
        this.$axios(`/api/paper/${paperId}`).then(res => {  //通过paperId获取试题题目信息
          ...
          keys.forEach(e => {   //foreach遍历keys中的每个元素，对于每个元素e执行下面的函数
            let data = this.topic[e]    //获取当前题型的数据数组
            this.topicCount.push(data.length)//获取当前题型的试题数量
            let currentScore = 0
          ...
          let dataInit = this.topic[1]//首先调出选择题然后显示出来
          this.number = 1
    ...
    getChangeLabel(val) { //获取选择题作答选项
      ...
      /* 保存学生答题选项 */
      this.topic1Answer[this.index] = val//根据当前index存储选择的选项
    },
