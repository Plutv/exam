...
<template>
  ...
  <div class="header">
    <div class="title">
      <span>{{examData.source}}</span><i class=""></i>
    ...
    init() {
      ...
      this.$axios(`/api/exam/${examCode}`).then(res => {  //通过examCode请求试卷详细信息
        ...
        this.$axios(`/api/paper/${paperId}`).then(res => {  //通过paperId获取试题题目信息
          this.topic = {...res.data}
          let keys = Object.keys(this.topic) //对象转成keys数组
          keys.forEach(e => {//forEach()遍历keys里每个元素，对于每个元素 e，执行函数体内的操作
            let data = this.topic[e]//获取当前题型的数据数组
            this.topicCount.push(data.length)//获取当前题目类型的题目长度
            let currentScore = 0
